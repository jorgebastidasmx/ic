...
PARAMS="-param:COD_PAIS=1 -param:FILE=$DIRJOB_GENERAR_SCORE -param:SCHEMA_EPM=EPM_MSG -param:KTR-LOAD=CREATE-FILES-DIARIO.ktr -param:KTR-SCORE=SCORE-1.ktr -param:KTR-RISK-PREDICTOR=SCORE-RISK-PREDICTOR.ktr  -param:KTR-MICROFINANZAS_2_0=MICROFINANZAS_2_0.ktr -param:PATH=$PATH_FILES -param:KTR-CLEAN-CACHE=CLEAN_CACHE.ktr" 
...
$DIRBINPTHO/kitchen.sh -file=$DIRJOB_GENERAR_SCORE/LOAD-SCORES-FROM-DATABASE.kjb $PARAMS -Level:Rowlevel >> $DIRLOG/Process_LoadScore$dia.log
