...
DIRJOB_GENERAR_BILLING=/opt/tools/Pentaho/EPM/LOAD_SCORE/JOB/
...
PARAMS="-param:COD_PAIS=1 -param:SCHEMA_EPM=EPM_MSG -param:PATH=$PATH_FILES -param:KTR-BILLING=LOAD_BILLING.ktr" 

echo $PARAMS
echo "Inicio proceso facturacion `date`" >> $DIRLOG/Process_Facturacion.log
$DIRBINPTHO/kitchen.sh -file=$DIRJOB_GENERAR_BILLING/LOAD-BILLING.kjb $PARAMS -Level:Rowlevel >> $DIRLOG/Process_Facturacion.log
